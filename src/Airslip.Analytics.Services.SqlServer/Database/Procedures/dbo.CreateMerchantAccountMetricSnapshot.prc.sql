...
BEGIN
    ...
    if not exists(select *
                  from MerchantAccountMetricSnapshots
        ...
        insert into MerchantAccountMetricSnapshots
        (IntegrationId, EntityId, AirslipUserType, MetricDate, Year, Month, Day, CurrencyCode, OrderCount, RefundCount,
         SaleCount, TotalRefunds, TotalSales)
        VALUES (@IntegrationId, @EntityId, @AirslipUserType, datefromparts(@Year, @Month, @Day), @Year,
    ...
    from MerchantAccountMetricSnapshots as mams
             join (select mt.IntegrationId,
                          mt.EntityId,
                          mt.AirslipUserType,
                          mt.Year,
                          mt.Month,
