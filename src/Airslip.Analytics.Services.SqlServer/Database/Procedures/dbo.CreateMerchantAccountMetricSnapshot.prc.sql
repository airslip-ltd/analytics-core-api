...
AS
Declare @Year as int, @Month as int, @Day as int, @IntegrationId as nvarchar(50), @CurrencyCode as nvarchar(3)

select @Year = Year, @Month = Month, @Day = Day, @IntegrationId = IntegrationId, @CurrencyCode = CurrencyCode
...
where Id = @Id
    ...
    using
        (select mt.IntegrationId,
         ...
         group by mt.IntegrationId, mt.EntityId, mt.AirslipUserType, mt.Year, mt.Month, mt.Day, mt.CurrencyCode) y
    ...
    when not matched then
        insert (IntegrationId, EntityId, AirslipUserType, MetricDate, Year, Month, Day, TotalSales, SaleCount,
                TotalRefunds,
        ...
        VALUES (y.IntegrationId, y.EntityId, y.AirslipUserType, datefromparts(y.Year, y.Month, y.Day), y.Year, y.Month,
                y.Day, y.TotalSales, y.SaleCount, y.TotalRefunds, y.RefundCount, y.OrderCount, y.CurrencyCode);
